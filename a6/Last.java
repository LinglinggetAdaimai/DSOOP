public class Last {
    ...
    public static Integer searchLastHelper(int[] a, int start, int end, int k){
        ...
        if(len == 1) {
            ...
            // add start to half because we're dealing with the real index (start, end)
            // if we only put len/2 you would go to like only the start of the list
            int half = start + len/2;
            if (a[half] == k && a[half + 1] != k) return half;
            if (a[half] > k) return  searchLastHelper(a, start, half, k);
